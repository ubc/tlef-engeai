 ...
 *
 * @deprecated Cutover complete — instructor/student features use scenario-questions-api.ts.
 * Kept for local demos only; do not import from feature modules.
 *
...
const E401_PART_A_ANSWER = `# Energy balance on water side
...
# Condensate flow from latent heat
...
# Area with assumed $U_{clean} = 850\\ \\mathrm{W\\,m^{-2}\\,K^{-1}}$
$A = Q / (U \\cdot \\text{LMTD})$
...
const E401_PART_B_ANSWER = `# Actual duty at 78 °C outlet
...
# Effective U from $Q = U A \\text{LMTD}$
...
# Ranked root causes (gradual 10-day decline, low outlet T)
...
const E401_PART_C_ANSWER = `# Verify readings
Cross-check outlet RTD with portable TC; confirm steam pressure and condensate trap operation.

# Inspect steam trap / vent
Eliminate air binding; ensure condensate removal.

# Schedule cleaning
Chemical or mechanical tube cleaning if fouling confirmed by $U_{eff}$ trend.

# Monitor
Log $T_{out}$, steam pressure, and calculated duty every 4 h for 48 h; plot $U_{eff}$ to confirm recovery after intervention.`;
...
Use this process sketch when reasoning about heat removal and lane uniformity:

<Artefact>
flowchart LR
  A[Product +5 C] --> B[Tunnel freezer]
  B --> C[Exit -18 C]
  D[Ammonia -35 C] --> B
</Artefact>`;
    ...
    return [
        {
            ...
            learningObjectives: [{ objectiveId: 'LO-10-1', text: 'LO-10-1', sourceTopicOrWeekId: topic1Id, sourceItemId: 'mock-item' }, { objectiveId: 'LO-10-2', text: 'LO-10-2', sourceTopicOrWeekId: topic1Id, sourceItemId: 'mock-item' }],
            subQuestions: [
                { subQuestionId: 'mock-sub-a', partId: 'a',
...
Start from the energy balance below:

<Artefact>
flowchart TD
  m[Mass flow 2400 kg/h] --> Q[Q = m cp DeltaT]
  cp[cp = 3.2 kJ/kgK] --> Q
  dT[DeltaT = 23 K] --> Q
  Q --> kW[Duty in kW]
</Artefact>`,
                    modelAnswer: `# Sensible-load duty
$$Q = \\dot{m} c_p \\Delta T = \\frac{2400}{3600} \\times 3200 \\times 23 \\approx 49.1\\ \\text{kW}$$

Add latent heat of freezing in a full design; this is a baseline sensible-load estimate.`
                },
                { subQuestionId: 'mock-sub-b', partId: 'b',
                    ...
                    modelAnswer: `# Airflow maldistribution
Most likely in tunnel freezers.

# Overloaded center conveyor spacing
Higher thermal load in center lanes.

# Frosted evaporator coils (center section)
Reduces heat transfer where frost builds first.

# Refrigerant feed imbalance
Uneven feed across coil circuits.`
                },
                { subQuestionId: 'mock-sub-c', partId: 'c',
                    subQuestionType: 'action',
                    prompt: 'Recommend immediate operational changes (no shutdown) to improve uniformity.',
                    modelAnswer: `# Reduce center loading
Reduce belt loading in center lanes.

# Verify airflow and defrost
Verify fan speeds and defrost schedule; check coil frost balance.

# Adjust spacing SOP
Adjust product spacing SOP for uniform residence time.`
                },
                { subQuestionId: 'mock-sub-d', partId: 'd',
                    subQuestionType: 'corrective',
                    prompt: 'If discharge pressure continues rising, what maintenance actions should be planned?',
                    modelAnswer: `# Condenser and charge
Inspect condenser fouling; check refrigerant charge and non-condensables.

# Compressor health
Verify compressor oil and suction superheat.

# Defrost audit
Schedule coil defrost performance audit.`
                }
            ],
        ...
        },
        {
            ...
            learningObjectives: [{ objectiveId: 'LO-10-1', text: 'LO-10-1', sourceTopicOrWeekId: topic1Id, sourceItemId: 'mock-item' }],
            subQuestions: [
                { subQuestionId: 'mock-sub-a', partId: 'a',
                    subQuestionType: 'calculation',
    ...
    switch (subQuestionType) {
        case 'calculation':
            return {
                ...
                modelAnswer: `# Identify knowns and assumptions
...
State which symptoms support each ranking.`
        ...
        case 'action':
            return {
                ...
                modelAnswer: `# Verify field readings
Cross-check critical instruments before major interventions.

# Stabilize operation
Adjust setpoints/loading within safe limits to protect downstream units.

# Inspect accessible items
Check traps, filters, bypasses, and valve positions relevant to ${equipment}.

# Monitor
Log key process variables every 4 h for 48 h and trend against design.`
            };
        case 'corrective':
            return {
                ...
                modelAnswer: `# Plan inspection
Plan controlled inspection during the next approved window.

# Cleaning or replacement
Schedule cleaning/replacement if fouling or wear is confirmed.

# Controls review
Review control strategy and alarm limits.

# Document and update PM
Document findings and update the preventive maintenance plan for ${equipment}.`
            };
        default:
            return {
