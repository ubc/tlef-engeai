 ...
 */
export const renderFlagSetup = (instructorCourse) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const html = yield loadComponentHTML('flag-setup');
        ...
        yield initializeFlagSetup(state, instructorCourse);
    }
    catch (error) {
        console.error("❌ Error during flag setup initialization:", error);
        yield showErrorModal("Initialization Error", "Failed to initialize flag setup. Please refresh the page and try again.");
    }
});
// ===========================================
 ...
 */
function initializeFlagSetup(state, instructorCourse) {
 ...
 */
function setupFlagCardListeners() {
    // Make demo functions globally available for tutorial
    window.toggleReportCard = toggleReportCard;
    window.handleDemoResolve = handleDemoResolve;
    window.toggleEditMode = toggleEditMode;
    window.saveEdit = saveEdit;
    window.handleDemoSave = handleDemoSave;
...
function toggleReportCard(card) {
    ...
    if (responseSection && responseSection.contains(event === null || event === void 0 ? void 0 : event.target)) {
        return;
    }
 ...
 */
function handleDemoResolve(button) {
    ...
    if (reportCard && emptyState) {
        ...
        const textarea = reportCard.querySelector('.response-textarea');
        const submittedResponse = textarea ? textarea.value : '';
        // Store the submitted response for Step 4
        window.submittedResponse = submittedResponse;
...
function toggleEditMode(button) {
    ...
    if (responseContent && editButton && saveButton) {
        ...
        (_a = responseContent.parentNode) === null || _a === void 0 ? void 0 : _a.replaceChild(textarea, responseContent);
        // Switch buttons
...
function saveEdit(button) {
    ...
    if (textarea && editButton && saveButton) {
        ...
        (_a = textarea.parentNode) === null || _a === void 0 ? void 0 : _a.replaceChild(responseContent, textarea);
        // Switch buttons
 ...
 * @param button - The save button that was clicked
 */
function handleDemoSave(button) {
    // Stop event propagation to prevent card collapse
    if (event) {
        event.stopPropagation();
    }
 ...
 */
function toggleFlagCard(headerElement) {
    const flagCard = headerElement.closest('.flag-card');
    if (!flagCard)
        return;
    const content = flagCard.querySelector('.flag-card-content');
    const chevron = flagCard.querySelector('.chevron-icon');
    if (!content || !chevron)
        return;
    // Toggle active class
 ...
 */
function setupDemoEditListeners() {
    ...
    if (saveBtn && editForm) {
        saveBtn.addEventListener('click', () => {
            ...
            const textarea = editForm.querySelector('.demo-response-input');
            if (textarea) {
...
function handleFinalCompletion(state, instructorCourse) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield showErrorModal("Completion Error", `Failed to complete flag setup: ${error instanceof Error ? error.message : 'Unknown error'}. Please try again.`);
        }
    });
}
// ===========================================
...
function markStepCompleted(stepNumber) {
    ...
    const state = window.flagSetupState;
    if (!state)
        return;
 ...
 */
function updateStep4WithSubmittedResponse() {
    const submittedResponse = window.submittedResponse;
