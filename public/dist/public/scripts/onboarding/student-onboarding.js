 ...
 */
export const renderStudentOnboarding = (user) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        window.studentOnboardingState = state;
        // Store user globally for access by flag submission handler
        window.currentOnboardingUser = user;
        ...
        const html = yield loadComponentHTML('student-onboarding');
        ...
        yield initializeStudentOnboarding(state, user);
    }
    catch (error) {
        console.error("❌ Error during student onboarding initialization:", error);
        yield showErrorModal("Initialization Error", "Failed to initialize student onboarding. Please refresh the page and try again.");
    }
});
// ===========================================
 ...
 * @param user - The student user object
 */
function initializeStudentOnboarding(state, user) {
 ...
 * @param user - The student user object
 */
function updateCourseDisplay(user) {
        ...
        try {
            ...
            try {
                ...
                if (typeof window.mermaid !== 'undefined') {
                    yield window.mermaid.init(undefined, `#${diagramId}`);
 ...
 * @param user - The student user object
 */
function bindEventListeners(state, user) {
    // Navigation buttons
    ...
    window.addEventListener('resize', () => {
        const state = window.studentOnboardingState;
        if (state) {
    ...
    const flagOtherInput = document.getElementById('flag-other-input');
    const flagOtherRadio = document.getElementById('flag-reason-5');
 ...
 * @param user - The student user object
 */
function handleBackClick(state, user) {
 ...
 * @param user - The student user object
 */
function handleNextClick(state, user) {
...
function handleFlagSubmission() {
    ...
    const otherInput = document.getElementById('flag-other-input');
    const statusMessage = document.querySelector('.flag-status-message');
    ...
    const otherDetails = (otherInput === null || otherInput === void 0 ? void 0 : otherInput.value.trim()) || '';
    ...
    if (flagType === 'other' && !otherDetails) {
        ...
        otherInput === null || otherInput === void 0 ? void 0 : otherInput.focus();
        return;
    }
    // Show loading state
    if (submitBtn) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            const state = window.studentOnboardingState;
            const user = window.currentOnboardingUser;
 ...
 */
function showFlagStatus(statusElement, message, type) {
    if (!statusElement)
        return;
...
function updateStepIndicators(state) {
    for (let i = 1; i <= state.totalSteps; i++) {
        ...
        const stepCircle = stepItem === null || stepItem === void 0 ? void 0 : stepItem.querySelector('.step-circle');
        if (stepCircle) {
 ...
 */
function updateSidebarStepIndicators(currentStepNumber) {
 ...
 */
function checkContentOverflow(stepElement) {
 ...
 * @param user - The student user object
 */
function handleOnboardingCompletion(user) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const response = yield fetch('/api/user/update-onboarding', {
                method: 'POST',
            ...
            yield showErrorModal("Completion Error", "There was an error completing your onboarding. Please try again.");
        }
    });
}
/**
...
function redirectToMainInterface() {
    ...
    delete window.studentOnboardingState;
    ...
    const event = new CustomEvent('onboarding-completed', {
        detail: { user: (_a = window.studentOnboardingState) === null || _a === void 0 ? void 0 : _a.user }
    });
...
function getHelpContent(stepNumber) {
    ...
    return helpContent[stepNumber] || "Help content not available for this step.";
}
// ===========================================
// UTILITY FUNCTIONS
// ===========================================
