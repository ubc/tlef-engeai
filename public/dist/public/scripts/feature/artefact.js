...
export class ArtefactHandler {
    ...
    setupEventDelegation() {
        ...
        document.addEventListener('click', (event) => __awaiter(this, void 0, void 0, function* () {
            ...
            const button = target.closest('.artefact-button');
            if (button) {
                ...
                if (buttonId && (buttonId.startsWith('artefact-btn-') || buttonId === 'demo-artefact-btn')) {
                    ...
                    yield this.toggleArtefact(artefactId);
                }
                return;
            }
            // Check for close button clicks
            const closeBtn = target.closest('#close-artefact-btn');
            if (closeBtn) {
                this.closeArtefact();
                return;
            }
            // Check for download button clicks
            const downloadBtn = target.closest('#download-artefact-btn');
            if (downloadBtn) {
                const panel = downloadBtn.closest('.artefact-panel');
                if (panel) {
                    // Find the currently open artefact
                    const openArtefact = Array.from(this.artefacts.values()).find(a => a.isOpen);
                    if (openArtefact) {
                        try {
                            yield this.downloadDiagram(panel, openArtefact);
                        }
                        catch (_a) {
                            yield showSimpleErrorModal('Failed to download diagram. Please try again.', 'Download Error');
                        }
                    }
                }
                return;
     ...
     */
    createArtefactButton(artefactData) {
...
graph TD
    ...
    `;
            ...
            if (this.currentlyOpenArtefactId === artefactId) {
                ...
                // Not open, so open it
                yield this.openArtefact(artefactId);
            }
        });
    }
    /**
    ...
    openArtefact(artefactId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let panel = document.getElementById('artefact-panel');
            if (!panel) {
            ...
            yield this.waitForPanelVisible(panel);
     ...
     */
    updateArtefactButton(artefactId) {
        const button = document.getElementById(`artefact-btn-${artefactId}`);
        if (!button)
            return;
        const artefactData = this.artefacts.get(artefactId);
        if (!artefactData)
     ...
     * @param artefactData - The artefact data
     */
    updateArtefactPanel(panel, artefactData) {
    ...
    formatMermaidCode(code) {
        try {
            ...
            formattedCode = formattedCode.replace(/(\w+)\[([^\]"']+[()][^\]"']*)\]/g, (match, nodeId, label) => {
    ...
    initializeMermaid(element) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const mermaid = yield this.waitForMermaid();
                // Ensure element is attached to DOM and visible
                yield this.ensureElementReady(element);
                ...
                if (mermaid.render) {
                    ...
                    const result = yield mermaid.render(id, mermaidCode);
     ...
     */
    showMermaidError(element, message) {
        element.innerHTML = `
            <div class="mermaid-error">
                ...
                <ul>
                    <li>Mermaid loaded: ${!!window.mermaid ? 'Yes' : 'No'}</li>
     ...
     * @param artefactData - The artefact data
     */
    setupPanZoom(panel, artefactData) {
        const container = panel.querySelector('.pan-zoom-container');
        const viewport = panel.querySelector('.mermaid-viewport');
     ...
     * @param artefactData - The artefact data
     */
    downloadDiagram(panel, artefactData) {
        return __awaiter(this, void 0, void 0, function* () {
            const mermaidElement = panel.querySelector('.mermaid');
            if (!mermaidElement) {
            ...
            try {
                ...
                yield this.downloadAsPNG(svgElement, artefactData.id);
            }
            catch (_a) {
                // Fallback to SVG export
                yield this.downloadAsSVG(svgElement, artefactData.id);
            }
        });
    }
    /**
     ...
     */
    downloadAsPNG(svgElement, artefactId) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => {
     ...
     * @param artefactId - The artefact ID
     */
    downloadAsSVG(svgElement, artefactId) {
        return __awaiter(this, void 0, void 0, function* () {
     ...
     */
    processStreamingText(text, messageId, onArtefactDetected) {
        try {
     ...
     */
    getArtefactsForMessage(messageId) {
        return Array.from(this.artefacts.values()).filter(artefact => artefact.messageId === messageId);
    }
}
ArtefactHandler.instance = null;
/**
 * Utility function to get artefact handler instance
 */
export function getArtefactHandler() {
