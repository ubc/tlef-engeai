...
import { openUploadModal, showSimpleErrorModal, showDeleteConfirmationModal, showSuccessModal, showErrorModal, showTitleUpdateLoadingModal, closeModal } from '../modal-overlay.js';
...
export function initializeDocumentsPage(currentClass) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        else if (typeof window !== 'undefined' && window.currentClass && window.currentClass.id) {
            courseId = window.currentClass.id;
            currentClass = window.currentClass; // Update currentClass reference
        }
        ...
        if (!courseId || courseId.trim() === '') {
            yield showSimpleErrorModal('Cannot load documents: Course ID is missing. Please refresh the page or return to course selection.', 'Initialization Error');
            return;
        }
        ...
        yield loadAllLearningObjectives();
         ...
         * @param currentClass the currently active class
         */
        function loadClassroomData(currentClass) {
        ...
        function syncCourseFromServer() {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    if (!courseId)
                        return;
                    const res = yield fetch(`/api/courses/${courseId}`);
                    if (!res.ok) {
         ...
         */
        function createDivisionElement(instance_topicOrWeek) {
            ...
            toggleInput.addEventListener('change', (e) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const response = yield fetch(`/api/courses/${courseId}/topic-or-week-instances/${instance_topicOrWeek.id}/published`, {
                        method: 'PATCH',
                        headers: {
                    ...
                    yield showSimpleErrorModal(errorMessage, 'Update Published Status Error');
                }
            }));
         ...
         * @param currentClass the currently active class
         */
        function updateDivisionButtonLabels(currentClass) {
        ...
        function setupEventListeners() {
            ...
            const existingHandler = container._documentsClickHandler;
            if (existingHandler) {
            ...
            const clickHandler = (event) => {
                ...
                // a) Pen icon (rename)
                const earlyRenameIcon = target.closest('.rename-icon');
                if (earlyRenameIcon) {
                    ...
                    const contentItem = earlyRenameIcon.closest('.content-item');
                    if (contentItem) {
                        const ids = contentItem.id.split('-'); // content-item-topicOrWeekId-contentId
                        if (!topicOrWeekId && ids[2])
                            topicOrWeekId = ids[2];
                        if (!itemId && ids[3])
                            itemId = ids[3];
                    }
                    if (!topicOrWeekId) {
                        const header = earlyRenameIcon.closest('.topic-or-week-header');
                        if (header)
                    ...
                    if (itemId) {
                        ...
                        const item = instance_topicOrWeek === null || instance_topicOrWeek === void 0 ? void 0 : instance_topicOrWeek.items.find(i => i.id === itemId);
                        if (item)
                ...
                const earlyOk = target.closest('.edit-ok-button');
                if (earlyOk) {
                    event.stopPropagation();
                    const contentItem = earlyOk.closest('.content-item');
                    if (contentItem) {
                        const ids = contentItem.id.split('-');
                        const topicOrWeekId = ids[2] || '0';
                        const itemId = ids[3] || '0';
                        const input = contentItem.querySelector('.title-edit-input');
                        if (!input)
                            return;
                        const newTitle = input.value.trim();
                        if (!newTitle) {
                            showErrorModal('Validation Error', 'Section name cannot be empty.');
                            return;
                        }
                        if (newTitle.length > 100) {
                            showErrorModal('Validation Error', 'Section name is too long (max 100 characters).');
                            return;
                        }
                        saveTitleChange(topicOrWeekId, itemId, newTitle);
                    }
                    else {
                        const header = earlyOk.closest('.topic-or-week-header');
                        if (!header)
                            return;
                        const topicOrWeekId = header.getAttribute('data-topic-or-week-instance') || '0';
                        const titleWrap = header.querySelector('.topic-or-week-title');
                        const input = titleWrap === null || titleWrap === void 0 ? void 0 : titleWrap.querySelector('.title-edit-input');
                        if (!input)
                            return;
                        const newTitle = input.value.trim();
                        if (!newTitle) {
                            showErrorModal('Validation Error', 'Division name cannot be empty.');
                            return;
                        }
                        if (newTitle.length > 100) {
                            showErrorModal('Validation Error', 'Division name is too long (max 100 characters).');
                            return;
                        }
                ...
                const earlyCancel = target.closest('.edit-cancel-button');
                if (earlyCancel) {
                    event.stopPropagation();
                    const contentItem = earlyCancel.closest('.content-item');
                    if (contentItem) {
                        const ids = contentItem.id.split('-');
                        const topicOrWeekId = ids[2] || '0';
                        const itemId = ids[3] || '0';
                        ...
                        const header = earlyCancel.closest('.topic-or-week-header');
                        if (!header)
                            return;
                        const topicOrWeekId = header.getAttribute('data-topic-or-week-instance') || '0';
                        const instance_topicOrWeek = courseData.find(d => d.id === topicOrWeekId);
                ...
                const divisionHeader = target.closest('.topic-or-week-header');
                if (divisionHeader) {
                ...
                const renameIconEl = target.closest('.rename-icon');
                if (renameIconEl) {
                    event.stopPropagation();
                    ...
                    const contentItem = renameIconEl.closest('.content-item');
                    if (contentItem) {
                        const ids = contentItem.id.split('-'); // content-item-topicOrWeekId-contentId
                        if (!topicOrWeekId && ids[2])
                            topicOrWeekId = ids[2];
                    ...
                    if (!topicOrWeekId) {
                        const header = renameIconEl.closest('.topic-or-week-header');
                        if (header) {
                            topicOrWeekId = header.getAttribute('data-topic-or-week-instance') || '';
                        }
                    }
                ...
                const okButtonEl = target.closest('.edit-ok-button');
                if (okButtonEl) {
                    event.stopPropagation();
                    // Determine context (division vs item) via closest containers
                    const contentItem = okButtonEl.closest('.content-item');
                    if (contentItem) {
                        // Item OK
                        const ids = contentItem.id.split('-'); // content-item-topicOrWeekId-contentId
                        const topicOrWeekId = ids[2] || '0';
                        ...
                        const header = okButtonEl.closest('.topic-or-week-header');
                        if (!header)
                            return;
                        const topicOrWeekId = header.getAttribute('data-topic-or-week-instance') || '0';
                        const titleWrap = header.querySelector('.topic-or-week-title');
                        ...
                        const newTitle = input.value.trim();
                        if (!newTitle) {
                            showErrorModal('Validation Error', 'Topic/Week name cannot be empty.');
                            return;
                        }
                        if (newTitle.length > 100) {
                            showErrorModal('Validation Error', 'Topic/Week name is too long (max 100 characters).');
                            return;
                        }
                        saveTitleChange(topicOrWeekId, null, newTitle);
                    }
                ...
                const cancelButtonEl = target.closest('.edit-cancel-button');
                if (cancelButtonEl) {
                    event.stopPropagation();
                    const contentItem = cancelButtonEl.closest('.content-item');
                    if (contentItem) {
                        const ids = contentItem.id.split('-');
                        const topicOrWeekId = ids[2] || '0';
                        const itemId = ids[3] || '0';
                        ...
                        const header = cancelButtonEl.closest('.topic-or-week-header');
                        if (!header)
                            return;
                        const topicOrWeekId = header.getAttribute('data-topic-or-week-instance') || '0';
                        const instance_topicOrWeek = courseData.find(d => d.id === topicOrWeekId);
                ...
                if (button) {
                    ...
                    if (action) {
                        ...
                        const headerElement = objectiveItem === null || objectiveItem === void 0 ? void 0 : objectiveItem.querySelector('.objective-header');
                        const topicOrWeekId = button.dataset.week || (headerElement === null || headerElement === void 0 ? void 0 : headerElement.dataset.topicOrWeekInstance) || '0';
                        const contentId = button.dataset.content || (headerElement === null || headerElement === void 0 ? void 0 : headerElement.dataset.content) || '0';
                        const objectiveIndex = parseInt((headerElement === null || headerElement === void 0 ? void 0 : headerElement.dataset.objective) || '-1', 10);
                        switch (action) {
                            case 'add':
                            ...
                            case 'delete-material':
                                ...
                                const additionalMaterialRow = button.closest('.additional-material');
                ...
                if (objectiveHeader) {
                    ...
                    if (!topicOrWeekId || !contentId || objectiveIndex < 0)
                        return;
            ...
            container._documentsClickHandler = clickHandler;
            ...
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    if (!courseId) {
                        ...
                        yield showSimpleErrorModal('Cannot add division: Course ID is missing.', 'Error');
                        return;
                    }
                    ...
                    const response = yield fetch(`/api/courses/${courseId}/topic-or-week-instances`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                    ...
                    if (response.status === 401 || response.status === 403) {
                        yield showSimpleErrorModal('You are not authorized to add divisions. Please sign in as an instructor.', 'Authorization Error');
                        return;
                    }
                    const result = yield response.json();
                    // console.log('📡 Add Division API Response body:', result); // 🔴 CRITICAL: API response data exposure
                    if (!result.success) {
                        yield showSimpleErrorModal('Failed to add division: ' + (result.error || 'Unknown error'), 'Add Division Error');
                        return;
                    }
                    const createdInstance = result.data;
                    // Update local state
                    ...
                    yield showSimpleErrorModal('An error occurred while adding the topic/week instance. Please try again.', 'Add Topic/Week Instance Error');
                }
            });
        }
        ...
        if (deleteAllDocumentsBtn) {
            // Remove any existing event listeners to prevent accumulation
            const existingDeleteHandler = deleteAllDocumentsBtn._deleteAllHandler;
            ...
            const deleteHandler = () => __awaiter(this, void 0, void 0, function* () {
                yield deleteAllDocuments();
            });
            // Store the handler reference and add the event listener
            deleteAllDocumentsBtn._deleteAllHandler = deleteHandler;
        ...
        */
        ...
        const addDivisionBtn = document.getElementById('add-division-btn');
        if (addDivisionBtn) {
            const existingAddDivisionHandler = addDivisionBtn._addDivisionHandler;
            ...
            const addDivisionHandler = () => __awaiter(this, void 0, void 0, function* () {
                yield addDivision();
            });
            addDivisionBtn._addDivisionHandler = addDivisionHandler;
         ...
         */
        function handleUploadMaterial(material) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    if (!topicOrWeekId) {
                        ...
                        yield showSimpleErrorModal('Topic/Week ID is missing. Please try again.', 'Upload Error');
                        return;
                    }
                    const instance_topicOrWeek = courseData.find(d => d.id === topicOrWeekId);
                    ...
                    if (!instance_topicOrWeek) {
                        ...
                        yield showSimpleErrorModal('Topic/Week instance not found. Please try again.', 'Upload Error');
                        return;
                    }
                    ...
                    if (!contentItem) {
                        ...
                        yield showSimpleErrorModal('Content item not found. Please try again.', 'Upload Error');
                        return;
                    }
                    if (!contentItem.additionalMaterials)
                        contentItem.additionalMaterials = [];
         ...
         * @returns null
         */
        function toggleDivision(topicOrWeekId) {
         ...
         * @returns null
         */
        function toggleObjectives(topicOrWeekId, contentId) {
         ...
         * @returns null
         */
        function toggleObjectiveItem(topicOrWeekId, contentId, index) {
         ...
         * @returns Promise<void>
         */
        function loadAllLearningObjectives() {
            return __awaiter(this, void 0, void 0, function* () {
                if (!currentClass)
                    return;
                try {
                    ...
                    for (const instance_topicOrWeek of courseData) {
                        for (const contentItem of instance_topicOrWeek.items) {
                            yield loadLearningObjectives(instance_topicOrWeek.id, contentItem.id);
                        }
                    }
                }
                catch (error) {
         ...
         * @returns Promise<void>
         */
        function loadLearningObjectives(topicOrWeekId, contentId) {
            return __awaiter(this, void 0, void 0, function* () {
                if (!courseId) {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        const instance_topicOrWeek = courseData.find(d => d.id === topicOrWeekId);
                        const content = instance_topicOrWeek === null || instance_topicOrWeek === void 0 ? void 0 : instance_topicOrWeek.items.find(c => c.id === contentId);
                        if (content) {
                        ...
                        yield showSimpleErrorModal('Failed to load learning objectives: ' + result.error, 'Load Learning Objectives Error');
                    }
                }
                catch (error) {
                    console.error('Error loading learning objectives:', error);
                    yield showSimpleErrorModal('An error occurred while loading learning objectives. Please try again.', 'Load Learning Objectives Error');
                }
            });
        }
        /**
         ...
         * @returns null
         */
        function addObjective(topicOrWeekId, contentId) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                const objectiveInput = document.getElementById(`new-title-${topicOrWeekId}-${contentId}`);
                if (!objectiveInput) {
                ...
                if (!learningObjective) {
                    yield showSimpleErrorModal('Please fill in the learning objective.', 'Validation Error');
                    return;
                }
                ...
                const newObjective = {
                    ...
                    courseName: currentClass.courseName,
                    topicOrWeekTitle: (instance_topicOrWeek === null || instance_topicOrWeek === void 0 ? void 0 : instance_topicOrWeek.title) || '',
                ...
                try {
                    if (!courseId) {
                        ...
                        yield showSimpleErrorModal('Cannot add learning objective: Course ID is missing.', 'Error');
                        return;
                    }
                    ...
                    if (result.success) {
                        ...
                        yield loadLearningObjectives(topicOrWeekId, contentId);
                        ...
                        yield showSimpleErrorModal('Failed to add learning objective: ' + result.error, 'Add Learning Objective Error');
                    }
                }
                catch (error) {
                    ...
                    yield showSimpleErrorModal('An error occurred while adding the learning objective. Please try again.', 'Add Learning Objective Error');
                }
            });
        }
        /**
         ...
         * @returns null
         */
        function editObjective(topicOrWeekId, contentId, index) {
            ...
            var _a, _b;
            const objective = (_b = (_a = courseData.find(d => d.id === topicOrWeekId)) === null || _a === void 0 ? void 0 : _a.items.find(c => c.id === contentId)) === null || _b === void 0 ? void 0 : _b.learningObjectives[index];
            if (!objective) {
            ...
            let contentDiv = document.getElementById(`objective-content-${topicOrWeekId}-${contentId}-${index}`);
            if (!contentDiv) {
                // If the target container doesn't exist (e.g., due to markup changes), create it on demand
                const headerEl = document.querySelector(`.objective-header[data-topic-or-week-instance="${topicOrWeekId}"][data-content="${contentId}"][data-objective="${index}"]`);
                const itemEl = headerEl === null || headerEl === void 0 ? void 0 : headerEl.parentElement; // .objective-item
                if (!itemEl) {
         ...
         * @returns null
         */
        function saveObjective(topicOrWeekId, contentId, index) {
            return __awaiter(this, void 0, void 0, function* () {
                var _a, _b;
                ...
                const learningObjective = document.getElementById(`edit-title-${topicOrWeekId}-${contentId}-${index}`).value.trim();
                if (!learningObjective) {
                    yield showSimpleErrorModal('Learning objective cannot be empty.', 'Validation Error');
                    return;
                }
                ...
                try {
                    if (!courseId) {
                        ...
                        yield showSimpleErrorModal('Cannot update learning objective: Course ID is missing.', 'Error');
                        return;
                    }
                    ...
                    const response = yield fetch(`/api/courses/${courseId}/topic-or-week-instances/${topicOrWeekId}/items/${contentId}/objectives/${objective.id}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                    ...
                    if (result.success) {
                        ...
                        yield showSimpleErrorModal('Failed to update learning objective: ' + result.error, 'Update Learning Objective Error');
                    }
                }
                catch (error) {
                    ...
                    yield showSimpleErrorModal('An error occurred while updating the learning objective. Please try again.', 'Update Learning Objective Error');
                }
            });
        }
        /**
         ...
         * @returns null
         */
        function cancelEdit(topicOrWeekId, contentId) {
            refreshContentItem(topicOrWeekId, contentId);
        }
        function deleteObjective(topicOrWeekId, contentId, index) {
            return __awaiter(this, void 0, void 0, function* () {
                var _a, _b;
                ...
                console.log('🔍 Current class ID:', currentClass === null || currentClass === void 0 ? void 0 : currentClass.id);
                console.log('🔍 Current class name:', currentClass === null || currentClass === void 0 ? void 0 : currentClass.courseName);
                //END DEBUG LOG : DEBUG-CODE(042)
                // Get the objective to show its name in confirmation
                const content = (_a = courseData.find(d => d.id === topicOrWeekId)) === null || _a === void 0 ? void 0 : _a.items.find(c => c.id === contentId);
                const objective = content === null || content === void 0 ? void 0 : content.learningObjectives[index];
                if (!objective) {
                ...
                const result = yield showDeleteConfirmationModal('Learning Objective', objective === null || objective === void 0 ? void 0 : objective.LearningObjective);
                ...
                if (result.action === 'delete') {
                    ...
                    const content = (_b = courseData.find(d => d.id === topicOrWeekId)) === null || _b === void 0 ? void 0 : _b.items.find(c => c.id === contentId);
                    const objective = content === null || content === void 0 ? void 0 : content.learningObjectives[index];
                    ...
                    try {
                        if (!courseId) {
                            ...
                            yield showSimpleErrorModal('Cannot delete learning objective: Course ID is missing.', 'Error');
                            return;
                        }
                        ...
                        if (result.success) {
                            ...
                            yield showSimpleErrorModal('Failed to delete learning objective: ' + result.error, 'Delete Learning Objective Error');
                        }
                    }
                    catch (error) {
                        ...
                        yield showSimpleErrorModal('An error occurred while deleting the learning objective. Please try again.', 'Delete Learning Objective Error');
                    }
                }
            });
        }
         ...
         * @returns null
         */
        function refreshContentItem(topicOrWeekId, contentId) {
            const instance_topicOrWeek = courseData.find(d => d.id === topicOrWeekId);
            const content = instance_topicOrWeek === null || instance_topicOrWeek === void 0 ? void 0 : instance_topicOrWeek.items.find(c => c.id === contentId);
            const itemContainer = document.getElementById(`content-item-${topicOrWeekId}-${contentId}`);
            if (!instance_topicOrWeek || !content || !itemContainer)
                return;
            ...
            const wasExpanded = (objectivesContent === null || objectivesContent === void 0 ? void 0 : objectivesContent.classList.contains('expanded')) || false;
         ...
         * @returns the created element
         */
        function buildContentItemDOM(topicOrWeekId, content) {
            ...
            if (materialsEl)
                item.appendChild(materialsEl);
            return item;
        }
        // ----- Sections management -----
        function addSection(instance_topicOrWeek) {
            return __awaiter(this, void 0, void 0, function* () {
                if (!currentClass) {
                ...
                try {
                    if (!courseId) {
                        yield showSimpleErrorModal('Cannot add section: Course ID is missing.', 'Error');
                        return;
                    }
                    ...
                    const response = yield fetch(`/api/courses/${courseId}/topic-or-week-instances/${instance_topicOrWeek.id}/items`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                    ...
                    if (result.success) {
                        ...
                        yield showSimpleErrorModal('Failed to add section: ' + result.error, 'Add Section Error');
                    }
                }
                catch (error) {
                    yield showSimpleErrorModal('An error occurred while adding the section. Please try again.', 'Add Section Error');
                }
            });
        }
        function deleteSection(topicOrWeekId, contentId) {
            return __awaiter(this, void 0, void 0, function* () {
                const instance_topicOrWeek = courseData.find(d => d.id === topicOrWeekId);
                if (!instance_topicOrWeek)
                    return;
                ...
                if (!content)
                    return;
                // Show confirmation modal
                const result = yield showDeleteConfirmationModal('Section', content.title || 'Section');
                ...
                if (!courseId) {
                    yield showErrorModal('Error', 'Course ID is missing. Cannot delete section.');
                    return;
                }
                try {
                    // Call backend API to delete the section
                    const response = yield fetch(`/api/courses/${courseId}/topic-or-week-instances/${topicOrWeekId}/items/${contentId}`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                    ...
                    if (resultData.success) {
                        ...
                        setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                            yield showSuccessModal('Success', 'Section deleted successfully.');
                        }), 100);
                    }
                    else {
                    ...
                    yield showErrorModal('Error', `Failed to delete section: ${errorMessage}`);
                }
            });
        }
        function updateDivisionCompletion(topicOrWeekId) {
            const instance_topicOrWeek = courseData.find(d => d.id === topicOrWeekId);
            if (!instance_topicOrWeek)
                return;
            const totalSections = instance_topicOrWeek.items.length;
            const container = document.querySelector(`.topic-or-week-header[data-topic-or-week-instance="${topicOrWeekId}"] .completion-status`);
            if (container)
                container.textContent = totalSections === 1 ? '1 section' : `${totalSections} sections`;
        }
        /**
         ...
         */
        function enterEditMode(topicOrWeekId, itemId, currentTitle) {
            let titleSpan = null;
            let titleContainer = null;
            if (itemId) {
                // Editing an item title
                titleSpan = document.querySelector(`#content-item-${topicOrWeekId}-${itemId} .content-title span`);
                titleContainer = document.querySelector(`#content-item-${topicOrWeekId}-${itemId} .content-title`);
            }
            else {
                // Editing a topic/week instance title
                titleSpan = document.querySelector(`.topic-or-week-header[data-topic-or-week-instance="${topicOrWeekId}"] .topic-or-week-title span`);
                titleContainer = document.querySelector(`.topic-or-week-header[data-topic-or-week-instance="${topicOrWeekId}"] .topic-or-week-title`);
            }
            ...
            const renameIcon = titleContainer.querySelector('.rename-icon');
            if (!renameIcon)
                return;
            ...
            okButton.addEventListener('click', (e) => __awaiter(this, void 0, void 0, function* () {
                e.stopPropagation();
                const newTitle = input.value.trim();
                // Validate input
                if (!newTitle) {
                    yield showErrorModal('Validation Error', `${itemId ? 'Section' : 'Division'} name cannot be empty.`);
                    input.focus();
                    return;
                }
                if (newTitle.length > 100) {
                    yield showErrorModal('Validation Error', `${itemId ? 'Section' : 'Division'} name is too long (max 100 characters).`);
                    input.focus();
                    return;
                }
                ...
                yield saveTitleChange(topicOrWeekId, itemId, newTitle);
            }));
            ...
            buttonContainer._escapeHandler = escapeHandler;
        }
        /**
         ...
         */
        function exitEditMode(topicOrWeekId, itemId, title) {
            let titleContainer = null;
            if (itemId) {
                titleContainer = document.querySelector(`#content-item-${topicOrWeekId}-${itemId} .content-title`);
            }
            ...
            const buttonContainer = titleContainer.querySelector('.edit-mode-buttons');
            if (!input || !buttonContainer)
                return;
            // Remove escape handler if exists
            const escapeHandler = buttonContainer._escapeHandler;
            if (escapeHandler) {
                document.removeEventListener('keydown', escapeHandler);
            }
            // Create title span
         ...
         */
        function saveTitleChange(topicOrWeekId, itemId, newTitle) {
            return __awaiter(this, void 0, void 0, function* () {
                var _a, _b;
                if (!currentClass) {
                ...
                try {
                    ...
                    if (itemId) {
                        ...
                        response = yield fetch(`/api/courses/${courseId}/topic-or-week-instances/${topicOrWeekId}/items/${itemId}/title`, {
                            method: 'PATCH',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                        ...
                        if (responseData.success) {
                            ...
                            if (item) {
                                ...
                                const updatedTitle = ((_a = responseData.data) === null || _a === void 0 ? void 0 : _a.title) || newTitle;
                        ...
                        response = yield fetch(`/api/courses/${courseId}/topic-or-week-instances/${topicOrWeekId}/title`, {
                            method: 'PATCH',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                        ...
                        if (responseData.success) {
                            ...
                            if (instance_topicOrWeek) {
                                // Update title from backend response if available
                                const updatedTitle = ((_b = responseData.data) === null || _b === void 0 ? void 0 : _b.title) || newTitle;
                    ...
                    yield showErrorModal('Error', errorMessage);
         ...
         */
        function renameDivision(instance_topicOrWeek) {
            return __awaiter(this, void 0, void 0, function* () {
                if (!currentClass) {
         ...
         */
        function renameItem(topicOrWeekId, item) {
            return __awaiter(this, void 0, void 0, function* () {
                if (!currentClass) {
         ...
         * @returns the created element
         */
        function createAdditionalMaterialsElement(content) {
         ...
         * @returns null
         */
        function deleteAdditionalMaterial(divisionId, contentId, materialId) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (!division) {
                    ...
                    yield showSimpleErrorModal('Division not found', 'Delete Material Error');
                    return;
                }
                const content = division.items.find(c => c.id === contentId);
                if (!content || !content.additionalMaterials) {
                    yield showSimpleErrorModal('Content not found', 'Delete Material Error');
                    return;
                }
                // Get the material to show its name in confirmation
                const material = content.additionalMaterials.find(m => m.id === materialId);
                const result = yield showDeleteConfirmationModal('Document', (material === null || material === void 0 ? void 0 : material.name) || 'this document');
                if (result.action === 'delete') {
                    try {
                        // Call backend API to soft delete from MongoDB
                        if (!courseId) {
                            console.error('❌ Cannot delete material: courseId is missing');
                            yield showSimpleErrorModal('Cannot delete material: Course ID is missing.', 'Error');
                            return;
                        }
                        const response = yield fetch(`/api/courses/${courseId}/topic-or-week-instances/${divisionId}/items/${contentId}/materials/${materialId}`, {
                            method: 'DELETE',
                            headers: { 'Content-Type': 'application/json' }
                        });
                        const result = yield response.json();
                        if (!result.success) {
                            yield showSimpleErrorModal('Failed to delete document: ' + result.error, 'Delete Document Error');
                            return;
                        }
                        ...
                        yield showSimpleErrorModal('An error occurred while deleting the document. Please try again.', 'Delete Document Error');
                    }
                }
                else {
        ...
        function createObjectivesListElement(topicOrWeekId, contentId) {
        ...
        function deleteAllDocuments() {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    // Show confirmation modal
                    const result = yield showDeleteConfirmationModal('All Documents', `all documents from the RAG database for course "${currentClass.courseName}"`);
                    if (result.action !== 'delete') {
                    ...
                    const response = yield fetch(`/api/rag/wipe-all?courseId=${courseId}`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                    ...
                    yield showSuccessModal(`Successfully wiped all documents from RAG database for course "${currentClass.courseName}"! Deleted ${((_a = result_data.data) === null || _a === void 0 ? void 0 : _a.deletedCount) || 0} documents.`, 'Database Wipe Success');
                }
                catch (error) {
                    console.error('Error deleting all documents:', error);
                    yield showSimpleErrorModal(`Failed to delete documents: ${error instanceof Error ? error.message : 'Unknown error'}`, 'Delete Error');
                }
            });
        }
    });
