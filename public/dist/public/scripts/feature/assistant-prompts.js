...
import { DEFAULT_PROMPT_ID } from '../../../src/functions/types.js';
 ...
 */
export function initializeAssistantPrompts(course) {
    return __awaiter(this, void 0, void 0, function* () {
        currentCourse = course;
        // Validate courseId
        if (!course.id || course.id.trim() === '') {
            console.error('❌ [ASSISTANT-PROMPTS] Cannot initialize: courseId is missing');
            yield showSimpleErrorModal('Cannot load assistant prompts: Course ID is missing. Please refresh the page.', 'Initialization Error');
            return;
        }
...
function loadPrompts() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const response = yield fetch(`/api/courses/${currentCourse.id}/assistant-prompts`, {
                method: 'GET',
            ...
            yield showErrorModal('Failed to load assistant prompts. Please try again.', 'Error');
        }
    });
}
/**
 ...
 */
function setupCardEventListeners(promptId) {
 ...
 * @param promptId - The prompt ID
 */
function handleSavePrompt(promptId) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        if (!(currentCourse === null || currentCourse === void 0 ? void 0 : currentCourse.id)) {
        ...
        try {
            if (isDraft) {
                ...
                const response = yield fetch(`/api/courses/${currentCourse.id}/assistant-prompts/${promptId}`, {
                    method: 'PUT',
                    credentials: 'same-origin',
                    headers: {
                        'Content-Type': 'application/json'
 ...
 * @param promptId - The prompt ID
 */
function handleCancelEdit(promptId) {
...
function handleDeletePrompt(promptId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const modalResult = yield showConfirmModal('Delete Prompt', `Are you sure you want to delete "${prompt.title}"?`, 'Delete', 'Cancel');
        ...
        try {
            ...
            if (!response.ok) {
                const errResult = yield response.json();
            ...
            showErrorToast(error instanceof Error ? error.message : 'Failed to delete prompt. Please try again.');
        }
    });
}
/**
...
function handleSelectPrompt(promptId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const modalResult = yield showConfirmModal('Select Prompt', `Set "${prompt.title}" as the active initial assistant prompt? This will replace any currently selected prompt.`, 'Select', 'Cancel');
        if (modalResult.action === 'cancel') {
            return;
        }
        try {
            const response = yield fetch(`/api/courses/${currentCourse.id}/assistant-prompts/${promptId}/select`, {
                method: 'POST',
                credentials: 'same-origin',
                headers: {
                    'Content-Type': 'application/json'
            ...
            showErrorToast(error instanceof Error ? error.message : 'Failed to select prompt. Please try again.');
        }
    });
}
/**
 ...
 * @param promptId - The prompt ID
 */
function handleToggleExpand(promptId) {
    const card = document.querySelector(`[data-prompt-id="${promptId}"]`);
    if (!card)
        return;
    const expandToggle = card.querySelector('.expand-toggle');
 ...
 */
export function resetUnsavedPromptChanges() {
    editingPromptId = null;
    draftPrompt = null;
}
