...
import { DEFAULT_BASE_PROMPT_ID, DEFAULT_LEARNING_OBJECTIVES_ID, DEFAULT_STRUGGLE_TOPICS_ID } from '../../../src/functions/types.js';
...
let draftItem = null;
let learningObjectivesCount = 0;
/**
 * Initialize the system prompts page
 * @param course - The current course
 */
export function initializeSystemPrompts(course) {
    return __awaiter(this, void 0, void 0, function* () {
        currentCourse = course;
        // Validate courseId
        if (!course.id || course.id.trim() === '') {
            console.error('❌ [SYSTEM-PROMPTS] Cannot initialize: courseId is missing');
            yield showSimpleErrorModal('Cannot load system prompts: Course ID is missing. Please refresh the page.', 'Initialization Error');
            return;
        }
        ...
        yield loadLearningObjectivesCount();
        // Load items from server
        yield loadSystemPrompts();
        // Render the page
        renderPrompts();
 ...
 */
function setupDefaultComponentEventListeners() {
    ...
    if (learningObjectivesCard) {
        const expandToggle = learningObjectivesCard.querySelector('.expand-toggle');
        if (expandToggle) {
    ...
    if (struggleTopicsCard) {
        const expandToggle = struggleTopicsCard.querySelector('.expand-toggle');
        if (expandToggle) {
 ...
 */
function loadLearningObjectivesCount() {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(currentCourse === null || currentCourse === void 0 ? void 0 : currentCourse.id))
            return;
        try {
...
function loadSystemPrompts() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const response = yield fetch(`/api/courses/${currentCourse.id}/system-prompts`, {
                method: 'GET',
            ...
            yield showErrorModal('Failed to load system prompts. Please try again.', 'Error');
        }
    });
}
/**
 ...
 */
function renderLearningObjectivesComponent() {
    // Build full content
 ...
 */
function renderStruggleTopicsComponent(struggleTopics) {
 ...
 */
function setupCardEventListeners(itemId) {
    const card = document.querySelector(`[data-prompt-id="${itemId}"]`);
    if (!card)
        return;
    ...
    if (titleField && isCurrentlyEditing) {
        titleField.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                contentField === null || contentField === void 0 ? void 0 : contentField.focus();
            }
...
function handleAddPrompt() {
    ...
    renderPrompts();
    setTimeout(() => {
        const newCard = document.querySelector(`[data-prompt-id="${DRAFT_PROMPT_ID}"]`);
        if (newCard) {
 ...
 * @param itemId - The item ID
 */
function handleEditPrompt(itemId) {
 ...
 * @param itemId - The item ID
 */
function handleSavePrompt(itemId) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        if (!(currentCourse === null || currentCourse === void 0 ? void 0 : currentCourse.id)) {
            showErrorToast('Course ID is missing. Please refresh the page.');
        ...
        const title = ((_a = titleField.textContent) === null || _a === void 0 ? void 0 : _a.trim()) || '';
        const content = ((_b = contentField.textContent) === null || _b === void 0 ? void 0 : _b.trim()) || '';
        if (!title) {
        ...
        try {
            if (isDraft) {
                ...
                const response = yield fetch(`/api/courses/${currentCourse.id}/system-prompts/${itemId}`, {
                    method: 'PUT',
                    credentials: 'same-origin',
                    headers: {
                        'Content-Type': 'application/json'
            ...
            showErrorToast(error instanceof Error ? error.message : 'Failed to save prompt. Please try again.');
        }
    });
}
/**
 ...
 * @param itemId - The item ID
 */
function handleDeletePrompt(itemId) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(currentCourse === null || currentCourse === void 0 ? void 0 : currentCourse.id)) {
            yield showErrorModal('Course ID is missing. Please refresh the page.', 'Error');
            return;
        }
        ...
        const result = yield showConfirmModal(`Are you sure you want to delete "${item.title}"?`, 'Delete Prompt', 'Delete', 'Cancel');
 ...
 */
function handleToggleAppend(itemId, append) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(currentCourse === null || currentCourse === void 0 ? void 0 : currentCourse.id)) {
            yield showErrorModal('Course ID is missing. Please refresh the page.', 'Error');
            return;
        ...
        const result = yield showConfirmModal(`${actionTextCapitalized} "${item.title}" ${append ? 'to' : 'from'} the system prompt? This will ${append ? 'add' : 'remove'} this component ${append ? 'to' : 'from'} all chat conversations.`, `${actionTextCapitalized} System Prompt Item`, actionTextCapitalized, 'Cancel');
        // Check if user cancelled
        ...
        try {
            const response = yield fetch(`/api/courses/${currentCourse.id}/system-prompts/${itemId}/append`, {
                method: 'POST',
                credentials: 'same-origin',
                headers: {
                    'Content-Type': 'application/json'
            ...
            showErrorToast(error instanceof Error ? error.message : `Failed to ${actionText} prompt. Please try again.`);
        }
    });
}
/**
...
function handleToggleExpand(itemId) {
    ...
    if (!card)
        return;
    const expandToggle = card.querySelector('.expand-toggle');
    const contentField = card.querySelector('.prompt-content');
    if (!expandToggle || !contentField)
        return;
 ...
 */
function handleDefaultComponentToggleExpand(componentId) {
    const card = document.querySelector(`[data-prompt-id="${componentId}"]`);
    if (!card)
        return;
    const expandToggle = card.querySelector('.expand-toggle');
    const contentField = card.querySelector('.prompt-content');
 ...
 */
export function hasUnsavedSystemPromptChanges() {
    return editingItemId !== null || draftItem !== null;
}
/**
 * Reset unsaved changes flag
 */
export function resetUnsavedSystemPromptChanges() {
    editingItemId = null;
    draftItem = null;
}
