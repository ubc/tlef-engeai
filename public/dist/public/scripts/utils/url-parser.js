 ...
 */
export function getInstructorViewFromURL() {
 ...
 */
export function buildInstructorURL(courseId, view, chatId) {
 ...
 */
export function navigateToInstructorView(view, chatId) {
 ...
 */
export function navigateToChat(courseId, chatId) {
 ...
 */
export function getStudentViewFromURL() {
    ...
    if (!pathMatch) {
        ...
        if (defaultMatch)
            return 'chat'; // Default to chat view
        return null;
    }
    const view = pathMatch[1];
    const validViews = ['chat', 'profile', 'flag-history', 'about'];
    return validViews.includes(view) ? view : null;
}
/**
 * Build student URL
 */
export function buildStudentURL(courseId, view, chatId) {
 ...
 */
export function navigateToStudentView(view, chatId) {
    const courseId = getCourseIdFromURL();
    if (!courseId) {
        console.error('[URL-PARSER] Cannot navigate: courseId not found in URL');
        return;
 ...
 */
export function navigateToStudentChat(courseId, chatId) {
 ...
 */
export function getInstructorOnboardingStageFromURL() {
    const pathMatch = window.location.pathname.match(/^\/course\/[a-f0-9]{12}\/instructor\/onboarding\/([^\/]+)/);
    if (!pathMatch)
        return null;
    const stage = pathMatch[1];
    const validStages = ['course-setup', 'document-setup', 'flag-setup', 'monitor-setup'];
 ...
 */
export function navigateToInstructorOnboarding(stage) {
    const courseId = getCourseIdFromURL();
    if (!courseId) {
        console.error('[URL-PARSER] Cannot navigate: courseId not found in URL');
        return;
 ...
 */
export function navigateToStudentOnboarding() {
    const courseId = getCourseIdFromURL();
    if (!courseId) {
        console.error('[URL-PARSER] Cannot navigate: courseId not found in URL');
        return;
 ...
 */
export function isNewCourseOnboardingURL() {
