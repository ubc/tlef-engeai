...
| Concern | Behavior |
|---------|----------|
| Breakpoints | Mobile-first; **centered** GitHub-style hero (copy above video) at all widths. Supporting extras (`#supporting`) use a **3-col hairline bento** at **768px** (stacks with horizontal dividers on small screens). Testimonials multi-column at **768px**. Team roster grid at **480px**. |
...
| Hero video | Centered full-width (max ~960px); 16:9 frame; green/blue **top-edge glow**. Scroll-buffer zoom (~135vh track after Learn more) is disabled under `prefers-reduced-motion`. |
| Features scrub | Sticky scroll-storytelling for three main features (`#features`) at **768px+**. UI stays **pinned** under the topbar while scroll advances steps 1→3 (then the page continues). Desktop: image-heavy two-column stage (`minmax(12rem, 0.75fr)` nav + `1.6fr` visual); full-height rail + moving active indicator; inactive titles muted, active title bold with body copy; right panels **opacity crossfade** on step change only (no scroll-linked zoom/fade). Feature images share a fixed **4:3** frame (`object-fit: cover`, `object-position: left center`) with a **4px** glass border and a **right-weighted** green glow (minimal top/bottom). Track height = `sticky-h + N × scroll-per-step + top spacing` (`--home-features-sticky-h` + `3 × 100vh` + `5rem`); JS pin range excludes `padding-top` and maps scroll to step index. `#home-more` reveal animation is **opacity-only** (no `transform` — that would break sticky). Marketing pages use `overflow-x: clip` on `html`/`body` (not `hidden`) and `overflow: visible` on `main` / `#home-more` so sticky is not broken. Below **768px** and under `prefers-reduced-motion`: static expanded list, no sticky trap. |
| Supporting extras | `#supporting` + `#funding` use `--home-promo-band-*`: **light** — CHBE green band (`--home-green`) with off-white type and white bento hairlines; **dark** — muted wash (`--home-surface-muted`, same as testimonials) with CHBE green hairlines (`--home-chbe-accent-line`). Feature images use a **16:9** frame with a **2px** glass border (`--home-support-graphic-border` / `--home-support-graphic-glass`). Shared accents: `--home-chbe-accent` / `--home-chbe-accent-line` also color topbar brand, testimonial card borders, and student status lines. |
| Home page assets | Under `public/assets/home/`: hero → `main-image.png`; features scrub → `feature-pathway.png`, `feature-scenario.png`, `feature-writing-feedback.png`; supporting extras → `feature-tone.png`, `feature-diagram.png`, `feature-multiple-modes.png`. Testimonials use `avatar-placeholder.svg` until real headshots exist. |
...
| Touch targets | Login / Learn more / Play / theme toggle / feature step triggers / grant options use ≥ ~44px targets. |

### Testing additions

- [ ] 375px: centered hero; video readable; CTAs wrap cleanly
- [ ] 375px: features section stacks statically (no sticky scrub); all 3 steps + images visible
...
- [ ] 1280px: features scrub — sticky pin holds under topbar; scroll through steps 1→2→3; click step jumps; panels opacity-crossfade on step change
- [ ] 1280px: supporting extras — 3 equal cells, shared hairlines, 16:9 images with 2px glass border
- [ ] Grant selector: underline tabs Year 1 → Year 2 → Total; dark + light readable
- [ ] Features: `prefers-reduced-motion` and phone widths show full static stack (no sticky scrub)
