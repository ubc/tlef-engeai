...
flowchart LR
  ...
  Service --> Canvas[Canvas gateways: synthetic mock and live OAuth release]
...
| 4. Intake and external boundaries | `document-extraction-service.ts`, `canvas-import-*.ts`, `canvas-release-service.ts`, `live-canvas-release-service.ts`, and `job-runner.ts` | Non-RAG extraction, synthetic/not-configured Canvas modes, preview-before-release, live exact-attempt release, and leased-job infrastructure |
...
- **Live mode** reads and writes through the signed-in staff member's Canvas OAuth client. Import remains read-only; release uses a separate preview and confirmation before attaching feedback PDFs or posting a grade.
...
|---|---|---|---|
| `rubricSource` | Writing | `internal_profile` \| `canvas` | A `canvas` writing rubric suppresses the metafunctions auto-fill, which assumes the built-in profile's criteria |
| `technicalRubricSource` | Technical | `canvas` \| `builtin` | Absent means the technical grid was seeded from the built-in lab profile |
| `canvasRubricImport` | Whichever lens the import seeded | `{ shape, ids, importedAt }` | `ids` are Canvas's own criterion/rating ids, and are the only way release can write points back into the instructor's Canvas rubric |

On a lab report, an imported Canvas rubric goes to the *technical* lens: it is the grid the assignment is graded on, and the writing lens keeps the SFL-grounded profile that generation depends on.

Saving a draft never changes generation, PDF, or release behavior. Approval promotes a version; an approved version is never edited in place. Runs and releases remain attributable to the approved rubric/profile version that governed them. Model grading suggestions remain staff-only and are not persisted. Release requires a complete staff-final per-criterion assessment saved with the latest review revision; the server validates that assessment against the exact rubric version and computes the total grade from criterion points.
...
5. A staff member reviews two tabs: **General Feedback** (rubric evidence, zero to two strengths, up to three revision priorities, deduplicated useful course-material labels, and a staff-final grade column beside staff-only model suggestions) and **Specific Feedback** (comments anchored to exact spans of the verified text). The specific working set seeds from the immutable model run's evidence quotes at read time; staff edit, delete, or add comments by selecting text, may attach an approved glossary entry snapshot, and every save appends a review revision snapshotting the full set and any complete staff-final assessment. The model run itself is never mutated.
...
8. The `src/report-generation` adapter renders student-safe PDFs. `include=general|annotated|both` selects the writing summary, the verified text with Canvas-style `/Highlight` popup annotations, or both. `lens=technical` renders the technical analysis, which is summary-only. A release renders one combined document per submission: for a lab report the technical analysis comes first, then the writing summary and annotations (D-095, superseding D-080). Staff may still download either lens on its own from the review page. No PDF includes `origin`, confidence, internal flags, model suggestions, or staff notes; the writing summary includes only the staff-final rubric assessment after staff save it.
9. Only an approved submission with a complete staff-final assessment may be released. Synthetic demo courses use the visibly labelled mock gateway. Live Canvas courses use `live-canvas-release-service.ts`: preview preflights grade and feedback, release verifies the exact Canvas submission attempt, uploads the approved combined PDF, attaches it in one submission comment, then posts the staff-final numeric grade through Canvas bulk grade update. The adapter still accepts a second technical artifact, which the orchestration layer does not currently send.

A release runs on the job queue rather than inside the request. `POST .../release` refuses what it can refuse cheaply — no preview, not approved, awaiting reconciliation, or five releases already spent — records the queuing staff member's `GlobalUser.userId` on the release record, and returns `202`. The worker reloads that record, rebuilds the same person's Canvas client from their stored credential (`queued-release-service.ts`), and writes in a fixed order: the feedback attachment (one combined PDF in one submission comment), then the per-criterion rubric assessment, then the total grade — which is skipped when the Canvas rubric grades the assignment, because the assessment has already set the grade and posting a total would overwrite a number Canvas derived itself. Each step is resumable from where the previous one stopped. Two states end the job without a retry: a release that already landed, and one parked in `reconciliation_required`, whose meaning is that nobody knows what Canvas accepted. A missing credential fails the job with a sanitized message naming reconnection as the fix; every other error is replaced with a generic one, since a provider error may quote submission text.

Feedback may be corrected and released again, up to five times per submission. The limit exists because each release adds a *new* Canvas submission comment and notifies the student, while the rubric and grade overwrite; five is enough for any genuine correction and low enough that a loop cannot bury a student in notifications. The count comes from the release records themselves and counts only what reached the student.

Release idempotency is keyed by a payload fingerprint that `canvas-release-service.ts` derives from the semantic release payload: submission, feedback run, rubric version, staff-final grade, final assessment details, optional technical run id, and the staff-approved student narrative. Rendered PDF bytes are prepared and retained server-side for live preview/release, but excluded from the durable fingerprint because each render stamps fresh annotation identifiers and timestamps. Including the narrative and final assessment is deliberate: a staff re-approval that changes student-facing text or grade is genuinely new content and must not collide with the earlier release.

### Batch generation

Staff can generate feedback for a whole assignment from its submission list (`batch-generation.ts`, `writing-feedback-batch.ts`). A preview classifies every active submission; a confirmation modal shows the counts, with an unticked option to regenerate feedback made with an older rubric version (staff comments are kept, but summary edits and revision goals are bound to the run they were written against and fall back to the new draft; the grade must be entered again for the new rubric regardless). Starting the batch queues one ordinary `generate` job per submission, so there is no batch record and the worker still drafts one submission at a time; it moves straight to the next job instead of waiting a polling interval. Stopping deletes the jobs that have not started.

File transcripts are part of the batch. `transcript-quality.ts` accepts extracted text only when it is non-empty, within the 30,000-character review limit, at least 50 words, has no more than three broken or private-use characters, is not letter-spaced, and is at least 70% letters and digits. Passing text is confirmed with `transcriptConfirmedBy: 'batch'`, and the review page tells staff that no person has checked it. Paper scans and failing extractions stay for staff to confirm by hand. This relaxes the rule that parsed file text is always staff-confirmed before generation, for born-digital files only; OCR output is still never accepted automatically.

Staff keep working while a batch runs. Review, redraft, and approval act on one submission at a time and are refused only for a submission that is `generating`: `appendReview` checks the status and the Mongo write repeats the check, and approval already requires `draft_ready`. Generation moves a submission to `draft_ready` (or `failed`) only from `generating`, so a submission stopped or deleted meanwhile is not relabelled. A failed attempt with retries left keeps the submission `generating`; the worker marks it `failed` only on the last attempt. Canvas release jobs are leased before generation jobs, so releasing approved feedback does not wait for the batch.

While a batch runs, deleting the assignment and switching a submission to a newer Canvas attempt are refused, as they are for any queued job; stopping the batch lifts that.
...
- Canvas OAuth release uses scoped staff credentials, redirect-safe upload, timeout reconciliation, and duplicate prevention. Uncertain comment or grade outcomes enter `reconciliation_required` rather than retrying automatically.
...
The current local vertical slice supports multiple assignment-specific rubrics, staff-approved SFL context profiles, instructor-approved directions, a visible synthetic Canvas browser/import, live Canvas import for linked courses, manual/digital intake, versioned rubric approval, async generation, structured SFL-founded linguistic feedback, a combined lab-report PDF that leads with the technical analysis, staff review, staff-final grading, mock release preview, and live Canvas feedback/grade release behind explicit preview and confirmation. LLED 200 is the first named pilot, not a hard-coded system subject. Production OCR, retention policy, real-student evaluation, Canvas rubric write-back, and governance approval for real student writing remain gated or deferred; do not describe this slice as production-ready.
