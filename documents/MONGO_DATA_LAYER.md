...
- `writing-submissions.transcriptConfirmedBy` is `staff` (set by `updateVerifiedWritingText`) or `batch` (set by `autoConfirmWritingTranscript`). It is absent on rows that never needed confirming and on rows confirmed before the field existed.
