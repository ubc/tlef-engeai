...
The landing is the assignment queue: a flat list of cards, one per assignment, each opening that assignment's own page. A card is a beige (`--background-2`) strip that is itself the control (`role="button"`, `aria-label`, `tabindex`, Enter/Space, a `chevron-right` affordance that is `aria-hidden`), and it carries only what identifies the assignment and what can be done to the assignment as a whole: title (with a **Lab report** chip when it is one), the origin-and-date line, a submission count, and an icon-only trash button (`createIconButton`, `wf-icon-button--danger`, feather `trash-2`, `aria-label`ed). Nothing on the queue expands; a card with a submission list inside it does not scale to a production course. With no assignments, show a plain muted empty note inviting Canvas import or manual creation; never render placeholder rows. Do not use uppercase eyebrow labels, navy headings, or card accent stripes anywhere — they are not part of the dashboard language.

## Assignment page

Everything that acts on one assignment lives on its own page, addressed by `?wfView=assignment&wfAssignment=<id>` so a reload or a shared link lands back on it. It opens with the shared sticky **← Back to assignments** bar, then the assignment's own name as the page title (`.wf-assignment-page-title`, matching the rubric page) with the origin/count/deadline meta line beneath it. Below the meta line, right-aligned on a row of their own, sit the three actions that act on the assignment as a whole, all in the workspace's white-pill `wf-header-btn` treatment. They stay on their own row rather than sharing the title's: opposite a long assignment name they wrapped below it anyway, and left-aligned themselves when they did, so their position depended on how the title happened to break. The actions are: **Generate feedback for all submissions** (only once there is at least one submission), **Sync submissions** (Canvas-linked assignments only), and **Edit rubric** (**View rubric** for TAs). Below them, submission rows sit in one bordered card (`.wf-submission-list`): student label, submitted date with a red **Late** text flag when past the deadline, attempt, tinted status chip, and a `.wf-submission-actions` group holding an icon-only trash button. The row itself opens the review (`role="button"`, Enter/Space, `chevron-right` affordance), and **+ Add submission (manually)** sits at the foot of the card, offering paste-text and file-upload in one inline form.

While a batch run is in flight, a tinted progress strip (`.wf-batch-bar`) appears at the head of that card carrying the running count and **Stop generating**, and the header's Generate action hides itself. The two are never on screen together: starting a run is meaningless while one is running, and a strip reporting nothing is an empty tinted band above the list. The strip is `hidden` otherwise, which its own `[hidden]` rule has to enforce over `display: flex`. The rubric and review pages return here rather than to the queue, so their back button reads **← Back to assignment**.

Both the assignment-card and submission-row deletes are icon-only trash buttons, never text buttons, and both remain confirm-modal-gated. Deleting an assignment is blocked (`409`) while it still has any submissions; the error surfaces through the existing action-error modal and the card stays in place. Deleting a submission is allowed at any status, including `released`; the confirmation modal adds an extra warning line for released submissions noting that only the local record is removed and the release cannot be recalled. Both confirmations use the shared modal helpers (`showDeleteConfirmationModal` / `showConfirmModal`), never a native `confirm()`.
...
- **Right — the Feedback panel.** A sticky panel in three steps, moved between with **← Back** / **Next →** around the step title:
  - **Step 1, Review annotations** (default) — FUNCTION (All/Content/Interpersonal/Organizational) and LEVEL (All/Text/Section/Clause & word) filter pill rows, then numbered annotation cards matching the document markers. Each card shows its number, function/level chips, a priority chip (high=red, medium=orange, low=green), an origin chip (**Model suggested** vs **Staff**), the anchored quote, **Feedback**, **Revision guidance**, an optional green-tinted **Suggested course material** box, and an optional glossary line. Cards open read-only with **Edit**/**Delete**; editing exposes the text fields plus Function/Level/Priority selects. Function/level/priority are staff triage metadata and never reach the student PDF. Filtered-out anchors dim in the document but stay clickable (clicking resets filters). Markers and cards stay in sync on hover/focus/click; markers are keyboard-activatable. Stale anchors (text re-verified after commenting) appear list-only with an amber treatment and a delete instruction.
  - **Step 2, Review feedback and grades** — editable strengths; one card per rubric criterion carrying the **Final grade** (required) above its editable feedback and evidence; readings; and editable priority revision goals, each with a Socratic **guiding question**. On the graded lens (technical for a lab report) each card's grade is a row of level buttons, each showing its point band, plus a points box. The points box has no visible label (its accessible name is "Points for <criterion>"). Clicking a single-value level fills its points; clicking a range level shows the range as the box's placeholder and "Enter exact points:" beside the box; points out of range show "Enter points from 0 to N:" there in red; typed points press the level they fall in. The model's suggested level carries an inset green edge and a "Suggested" tag and is never pre-selected. The card header chip reads "Not graded", the points, or "Check points". A lab report's Writing tab says it is not graded.
  - **Step 3, Review and release** — a release callout once feedback has gone to Canvas (or needs reconciliation); a read-only grade summary (level chip, points, and **Edit**/**Grade** links back to the card in Step 2) with the total; **Open full rubric grid** (read-only; the rubric as the student's PDF prints it, each cell with its rating name, point band and original description, the model's suggested cell tagged "Suggested" and the cell the final grade falls in filled green); **Preview feedback PDF**, which opens the exact document a release attaches (for a lab report, the technical feedback first); the internal-note editor; review history; and, last, a **Connect Canvas** callout when this staff member has not yet authorized Canvas (it returns to Step 3 of the same submission, after the usual unsaved-changes warning).
- The panel footer shows on Steps 2 and 3: a sentence saying what can happen next, **Save draft** (one save covers both lenses, the annotation working set, the note and any grades entered so far), and on Step 3 a single primary action — **Approve** (disabled until every criterion has valid points; it saves unsaved edits first), then **Release to Canvas** once approved. Saving an approved submission asks first, because it withdraws approval.

**Review history** is a full, read-only audit trail, not an undo control. It sits closed at the foot of Step 3 as a single "Review history (N)" row. Opened, each saved revision renders as a collapsible entry (all closed by default) labelled with the revision number, timestamp, and the roster name of the staff member who saved it ("Course staff" when they are no longer on the course's instructor or TA list; the internal user id is never shown). Expanding an entry shows the exact student-facing feedback and internal note text as saved, plus a comment-level diff against the previous revision: **Added**, **Edited**, and **Removed** lines, each with the anchored quote, comment text, and an origin chip. There is no restore/revert action anywhere in this panel.

The explicit actions are Confirm transcript, Generate feedback, Save draft, Approve, PDF viewers (summary, annotated, complete, technical), and Release to Canvas. Generation must never imply that the work was graded or released.
...
Never conflate **Approve** with **Release to Canvas**. Approve takes effect in one click, because it writes nothing outside EngE-AI; Release always asks for confirmation. A successful release preview updates an inline preview region; it is not a confirmation dialog and it does not release anything.
