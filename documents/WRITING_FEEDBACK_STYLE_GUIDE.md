...
- **Right — the Feedback panel.** A sticky panel in three steps, moved between with **← Back** / **Next →** around the step title:
  ...
  - **Step 2, Review feedback and grades** — a pinned progress line ("Grades 2 of 4 · 25 / 50 so far", with a jump to the next ungraded criterion); editable strengths; one card per rubric criterion carrying the **Final grade** (required) above its editable feedback and evidence; readings; internal flags; and editable priority revision goals, each with a Socratic **guiding question**. On the graded lens (technical for a lab report) each card's grade is a row of level buttons, each showing its point band, plus a points box. Clicking a single-value level fills its points; clicking a range level asks for the number; typed points press the level they fall in. The model's suggested level carries an inset green edge and a "Suggested" tag and is never pre-selected. The card header chip reads "Not graded", the points, or "Check points". A lab report's Writing tab says it is not graded.
  - **Step 3, Review and release** — a release callout once feedback has gone to Canvas (or needs reconciliation); a read-only grade summary (level chip, points, and **Edit**/**Grade** links back to the card in Step 2) with the total; **Open full rubric grid** (read-only); the student PDF viewers; the internal-note editor; review history; and, last, a **Connect Canvas** callout when this staff member has not yet authorized Canvas (it returns to Step 3 of the same submission, after the usual unsaved-changes warning).
