...
| GET | `/submissions/:submissionId/feedback.pdf` | Student-safe feedback PDF; `?include=general\|annotated\|both` selects the writing summary document, the verified text with Canvas-style `/Highlight` popup annotations, or both (default `general`; legacy `specific` maps to `annotated`). `?lens=technical` returns the separate technical summary PDF for lab-report assignments. Served `inline` so staff can read what they just wrote; `?download=1` is the explicit save |
