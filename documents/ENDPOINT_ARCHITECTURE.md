...
| GET | `/submissions/:submissionId` | Submission, history, latest feedback run, optional latest release state, stored anchored `comments` (stale-flagged against the current verified text), and `seedComments` derived from run evidence while no revision has stored comments |
