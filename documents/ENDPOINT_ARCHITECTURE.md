...
| GET | `/assignments/:assignmentId/rubric` | Returns optional approved rubric, current draft (omitted when its content matches the approved rubric, which is no unapproved change), immutable history, optional criterion library (linguistic lens only), and caller edit permission for the selected lens. `?lens=linguistic\|technical` selects which rubric is read (default `linguistic`); requesting `technical` on an assignment not marked as a lab report is `409` |
