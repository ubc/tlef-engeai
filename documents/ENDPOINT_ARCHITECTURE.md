...
| GET | `/submissions/:submissionId/feedback.pdf` | Student-safe feedback PDF; `?include=general\|annotated\|both` selects the writing summary document, the verified text with Canvas-style `/Highlight` popup annotations, or both (default `general`; legacy `specific` maps to `annotated`). `?lens=technical` returns the separate technical summary PDF for lab-report assignments |
| POST | `/submissions/:submissionId/release-preview` | Dry-run release preview. Mock courses validate the local payload; live Canvas courses preflight the exact staff-final grade and feedback PDF upload without writing |
| POST | `/submissions/:submissionId/release` | Explicit release after approval and matching preview. Mock courses use the synthetic gateway. Live Canvas verifies the exact Canvas submission attempt, attaches the approved writing PDF plus optional technical PDF in one comment, then posts the staff-final grade through Canvas bulk grade update |
