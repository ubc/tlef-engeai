...
| File | Change |
|---|---|
| `writing-feedback-review.ts:1327` | Branch on `definition?.assessedBy === 'staff'` before the missing-feedback branch. Render the criterion card with the grade control plus `summaryEditor.explanationField(...)` seeded from the saved edit. The card header carries a "Written by staff" chip, and the required Feedback box shows "EngE-AI doesn't assess this criterion. Write the feedback for the student here." as its placeholder and accessible description |
